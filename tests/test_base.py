...
class BaseXRayLabToolTest(ABC):
    """
    Abstract base class for all XRayLabTool tests.
...
def generate_test_materials(count: int = 5) -> list[tuple[str, float]]:
...
def generate_test_energies(
    energy_range: tuple[float, float] = (1.0, 30.0),
    num_points: int = 50,
    spacing: str = "linear",
) -> np.ndarray:
