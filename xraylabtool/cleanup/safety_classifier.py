...
class SafetyClassifier:
    ...
    def classify(self, file_path: Path) -> ClassificationResult:
        ...
        critical_match = self._check_pattern_groups(file_path, self._compiled_patterns['critical'])
        if critical_match:
            return ClassificationResult(
        ...
        if suspicious_matches:
            warnings.append(f"Suspicious patterns found: {', '.join(suspicious_matches)}")
            confidence = max(0.1, confidence - 0.3)  # Reduce confidence

        # 4. Check safe patterns
        safe_match = self._check_pattern_groups(file_path, self._compiled_patterns['safe'])
        if safe_match:
        ...
        if self.strict_mode and confidence > 0.8 and category == FileCategory.SAFE_TO_REMOVE:
