...
class ArchitectureAuditor:
    ...
    def __init__(self, docs_directory: Path):
        ...
        self.recommended_patterns = {
            "calculate_single_material_properties": "from xraylabtool.calculators import calculate_single_material_properties",
            "calculate_xray_properties": "from xraylabtool.calculators import calculate_xray_properties",
            "XRayResult": "from xraylabtool.calculators.core import XRayResult",
            "parse_formula": "from xraylabtool.utils import parse_formula",
            "energy_to_wavelength": "from xraylabtool.utils import energy_to_wavelength",
        }
    ...
    def audit_module_coverage(self) -> List[Dict]:
        ...
        for doc_file in doc_files:
            ...
            if any(skip in str(doc_file) for skip in ['cleanup-system', 'optimization', 'physics']):
                continue

            # Count mentions of current modules
            module_mentions = {}
            for module, description in self.current_modules.items():
                count = len(re.findall(r'\b' + re.escape(module) + r'\b', content))
                if count > 0:
                    module_mentions[module] = count

            if module_mentions:
                architecture_coverage[str(doc_file.relative_to(self.docs_dir.parent))] = module_mentions
        ...
        for key_file in key_arch_files:
            ...
            if file_path not in architecture_coverage:
                ...
                violations.append({
                    'file': file_path,
                    'issue': f'Limited architecture coverage: only {len(architecture_coverage[file_path])} modules mentioned',
                    'modules_mentioned': list(architecture_coverage[file_path].keys()),
    ...
    def audit_code_examples(self) -> List[Dict]:
        ...
        for doc_file in doc_files:
            ...
            code_blocks = re.findall(r'.. code-block:: python\s*\n\n(.*?)\n\n', content, re.DOTALL)
            code_blocks.extend(re.findall(r'```python\s*\n(.*?)\n```', content, re.DOTALL))
